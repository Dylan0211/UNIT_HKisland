...
def create_a_and_b_weekday(df, context_a, context_b):
    ...
    for i in range(len(context_a_each_weekday_data)):  # every weekday data
        ...
        for j in range(len(context_a_each_weekday_data[i])):
            ...
            context_a_data.append(np.array(df.loc[this_index: this_index + 23, 'nor_cl']))
        context_b_data = []
        for j in range(len(context_b_each_weekday_data[i])):
            this_index = df[df['time'] == context_b_each_weekday_data[i][j]].index[0]
            context_b_data.append(np.array(df.loc[this_index: this_index + 23, 'nor_cl']))

        # get mappings
        for j in range(len(context_a_data)):
            for k in range(len(context_b_data)):
                cl_a.append(context_a_data[j])
                cl_b.append(context_b_data[k])

                # check
                # fig = plt.figure(figsize=(10, 6))
                # fig.add_subplot(111)
                # plt.plot(range(context_a_data[j].shape[0]), context_a_data[j], color='b', label='a')
                # plt.plot(range(context_b_data[k].shape[0]), context_b_data[k], color='orange', label='b')
                # plt.legend()
                # plt.show()
...
def read_data():
    ...
    with open('./tmp_pkl_data/{}_ashrae_{}_to_{}_cl.pkl'.format(read_data_building_name, context_a, context_b), 'wb') as w:
        pickle.dump((cl_a, cl_b, load_max, load_min, df), w)
    print('data saved at: ./tmp_pkl_data/{}_ashrae_{}_{}_cl.pkl'.format(read_data_building_name, context_a, context_b))
