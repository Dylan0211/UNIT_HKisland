...
def test_model():
    ...
    for model_name in model_name_list:
        # load data
        with open('./tmp_pkl_data/{}_one_month_ashrae_{}_X_Y_max_min.pkl'.format(building_name, ashrae), 'rb') as r:
    ...
    # output as csv file
    with open('./test_result/test_on_{}_one_month_ashrae_{}_result.csv'.format(building_name, ashrae), 'w', newline='') as f:
        write = csv.writer(f, dialect=('excel'))
        write.writerow(['rank', 'model_name', 'MAE'])
        for index, model in enumerate(sorted_models):
            write.writerow([index + 1, model[0], model[1]])
    print('result saved at: ./test_result/test_on_{}_one_month_ashrae_{}_result.csv'.format(building_name, ashrae))
