...
def create_a_and_b(df, context_a, context_b):
    ...
    # check data a and b
    check_a = df_context_a.loc[:24 * num_data - 1, 'coolingLoad'].tolist()
    check_b = df_context_b.loc[:24 * num_data - 1, 'coolingLoad'].tolist()
    ...
    # get nor_cl column (seq_length = 24)
    x_a = []
    x_b = []
    for j in range(num_data):
        x_a.append(np.array(df_context_a.loc[24 * j: 24 * (j + 1) - 1, 'nor_cl']))
        x_b.append(np.array(df_context_b.loc[24 * j: 24 * (j + 1) - 1, 'nor_cl']))

    # get mappings between a and b
    final_x_a = []
    final_x_b = []
    for k in range(len(x_a)):
        for l in range(len(x_b)):
            final_x_a.append(x_a[k])
            final_x_b.append(x_b[l])
    final_x_a = np.array(final_x_a)
    final_x_b = np.array(final_x_b)
    return final_x_a, final_x_b
...
def read_data(building_name):
    ...
    # create a and b
    context_a = 1  # winter average weekday
    context_b = 4  # summer average weekday
    x_a, x_b = create_a_and_b(df=df, context_a=context_a, context_b=context_b)

    # save
    print('x_a shape: {}'.format(x_a.shape))
    print('x_b shape: {}'.format(x_b.shape))
    with open('./tmp_pkl_data/{}_a_b_max_min_context.pkl'.format(building_name), 'wb') as w:
        pickle.dump((x_a, x_b, load_max, load_min, df), w)
